...
**Not verified by Claude this session**: the ticket's own falsifier (`getBoundingClientRect().bottom ≤ innerHeight` measured via Playwright) — this environment's Playwright browser isn't installed (`npx playwright install chrome` needed). Flagged honestly rather than claimed; Gavi's own live-testing substituted for it, confirmed item-by-item.
