...
- **Frontend:** React (Vite), **JavaScript only — no TypeScript** (Gavi doesn't know TS yet; planned to learn it after this project). RTL/Hebrew-ready from day one (logical CSS properties); content English-only initially. Every "build a page/component" task includes a real design pass, not just logic-wiring to endpoints — see Design system below.
