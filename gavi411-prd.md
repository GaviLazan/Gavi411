...
- **Frontend:** React (Vite), **JavaScript only — no TypeScript** (Gavi doesn't know TS yet; planned to learn it after this project). UI is English-only/LTR; Hebrew only ever appears as user-entered content (request text, messages), which needs correct bidi text rendering (mixed Hebrew/English/numbers) on those specific fields — not page-layout RTL support. Every "build a page/component" task includes a real design pass, not just logic-wiring to endpoints — see Design system below.
