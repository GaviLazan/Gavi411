...
export async function sendNudge(requestId, admin = null) {
  ...
  const request = await prisma.request.findUnique({
    where: { id: requestId },
    select: { nudgedAt: true },
  })
  if (request?.nudgedAt !== null) {
...
export async function runAutoCloseCheck() {
  const nudgedRequests = await prisma.request.findMany({
    ...
    select: { id: true, nudgedAt: true, createdAt: true },
  })
  ...
  for (const req of nudgedRequests) {
    ...
    // Check if nudge #2 has already been sent this cycle (isSystem message
    // with NUDGE_TWO_TEXT created after nudgedAt)
    const nudgeTwoSent = await prisma.message.findFirst({
      where: {
        requestId: req.id,
        isSystem: true,
        content: NUDGE_TWO_TEXT,
        createdAt: { gte: req.nudgedAt },
      },
    })

    if (nudgeTwoSent && inactiveMs >= CLOSE_AFTER_MS) {
    ...
    } else if (!nudgeTwoSent && inactiveMs >= NUDGE_TWO_AFTER_MS) {
