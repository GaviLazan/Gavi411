...
**Also decided this session**: G411-116 is one Jira issue covering 5 separate PRs (server routes · server lib+middleware · client pages · client components+lib · tests), built one at a time per Gavi's explicit choice over defining all 5 splits up front. Each PR gets its own STOP-3 walkthrough and its own merge ask, but the ticket itself should NOT transition to Reconciled until all 5 PRs have landed — a single PR's merge only closes that PR's own slice of scope, not the ticket. Falsifier/Evidence-bar-met on the Jira issue get updated per-PR-pickup (scoped to what's actually in flight), not rewritten wholesale until the final PR closes it out.
