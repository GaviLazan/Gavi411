...
**The prior entry below (and brain.md #167) claimed these three "stay at root... because impeccable's own tooling reads them from root by convention" — never actually verified, and wrong.** Checked `.claude/skills/impeccable/scripts/context.mjs`'s real path-resolution logic before assuming a config change was needed: its own `FALLBACK_DIRS = ['.agents/context', 'docs']` already treats `docs/` as a supported fallback location. Confirmed live — `node context.mjs --target client/src` resolved `docs/PRODUCT.md`/`docs/DESIGN.md` correctly with zero changes to `.impeccable/config.json` or any script. Logged the correction as brain.md #169, and edited #167's own text in the same pass rather than leaving the wrong claim to be found stale later.
...
**../../docs/DESIGN.md refreshed, not rewritten**: per Gavi's explicit "refresh" choice when asked (existing file, last written Sep 20). Updated only what actually drifted — the new sage-green values with a note on why, a line confirming purple was never a real button variant, and the reduced-motion bullet now correctly includes the credit ring. `.impeccable/design.json` sidecar's `sage-green` colorMeta/tonalRamp and `generatedAt` synced to match; the rest of that file got incidentally re-indented by the write (Python's `json.dump`, not a content change) — flagged to Gavi rather than silently letting the diff look bigger than it is.
...
### What's next, concretely
1. **WP12 (close-out)** is still the next real work package per the finish-line plan, untouched. Remaining parts: design re-critique (`/impeccable critique` + `/impeccable audit`), a live phone-in-hand pass on the deployed app, a docs refresh (`../../docs/DESIGN.md` via `/impeccable document`), and demo prep. Confirm scope with Gavi at STOP 1 before starting.
...
4. **WP12 (close-out)** is still the next real work package per the finish-line plan — untouched this stretch. Its Jira-reconciliation step is already done (G411-7/8/9 all Reconciled). Remaining WP12 parts: design re-critique (`/impeccable critique` + `/impeccable audit`), a live phone-in-hand pass on the now-actually-working deployed app (this is a good moment for it, given the Telegram/push fixes), a docs refresh (`../../docs/DESIGN.md` via `/impeccable document`), and demo prep (seed a real friend account with 2-3 realistic requests, pre-warm Render before presenting). Confirm scope with Gavi at STOP 1 before starting, per usual.
...
### What's next, concretely
1. **WP12 (close-out)** is next per the finish-line plan — but note its own Jira step (reconcile G411-7/8/9) is now already done ahead of time, so WP12 picked up next only needs its other 4 parts: design re-critique (`/impeccable critique` + `/impeccable audit`), a live phone-in-hand pass on the deployed Vercel+Render build, a docs pass (../../docs/DESIGN.md refresh via `/impeccable document`), and demo prep (seed a real friend account with 2-3 realistic requests, pre-warm Render). Confirm with Gavi at STOP 1 before starting — a Reconciled epic doesn't imply "start WP12" automatically.
...
### Real state, right now
Merged onto `main` (regular merge commit, per this project's standing no-squash convention). Primary worktree back on `main`, matching `origin/main`. `gavi411-brain.md` decision #149 logged (ghost-button border, ../../docs/DESIGN.md updated in the same pass). Two throwaway test requests created during automated verification were deleted from the DB afterward. Dev servers (Vite :5173, API :3000) still running from this session.
...
2. **Presence dot invisible when offline, in both themes.** `--border` (1.17:1 light / 1.47:1 dark against the page background) is by design a barely-there hairline color per ../../docs/DESIGN.md, never meant to carry a status signal — that's the actual bug, not a missing dark-mode value. Fixed with `--danger` (6.26:1 / 7.78:1), the same token `StatusChip`'s HIGH-urgency dot already uses. Verified live against a real "offline" account, not simulated.
...
2. **Stale `../../docs/DESIGN.md` content Haiku's own edit missed**: a whole Lavender/`button-purple` section (frontmatter YAML block + prose) still described lavender as live even though this same PR deletes `.btn-purple` from the CSS. Fixed the frontmatter and forward-looking prose; left the historical "this direction was revised..." narrative alone (decision record, not current guidance).
...
A `[impeccable@1]` design-system hook flagged 3 pre-existing lines in `index.css` (h2's responsive 20px, `code`'s 4px radius, `code`'s 15px font) as outside `../../docs/DESIGN.md`'s documented scale — confirmed via `git diff main` that none are touched by this PR's diff at all. Left alone, not ignored via `hook-admin.mjs` either (not confident enough to call them sanctioned exceptions without a real audit) — flagged here as an open item for whoever next does design-system work. The hook also wants `/impeccable document` re-run since `../../docs/DESIGN.md` changed — not run this session, not this ticket's job.
...
3. Open item, not blocking: the 3 pre-existing `index.css` design-hook findings above, and the `/impeccable document` sidecar refresh — whoever next touches `index.css`/`../../docs/DESIGN.md` for real should pick these up.
